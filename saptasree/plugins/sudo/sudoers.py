...
from saptasree.utils.database import add_sudo, remove_sudo
...
async def userdel(client, message: Message, _):
    ...
    if user.id == PROTECTED_ID:
        keyboard = InlineKeyboardMarkup(
            [
                [
                    InlineKeyboardButton("🥲 sad", callback_data="sad_cb"),
                    InlineKeyboardButton("✅ ok", callback_data="ok_cb"),
                ]
            ]
        )
        return await message.reply_text(
            "<b>ᴋɴᴏᴡ ᴡʜᴀᴛ ᴀʀᴇ ʏᴏᴜ ᴅᴏɪɴɢ ʙᴇғᴏʀᴇ ᴅᴏɪɴɢ ɪᴛ</b> ❗️",
...
@app.on_callback_query(filters.regex("ok_cb"))
async def ok_response(_, query: CallbackQuery):
    await query.message.edit_text("<b>nice try nigga 🥹 </b>")


@app.on_callback_query(filters.regex("sad_cb"))
async def sad_response(_, query: CallbackQuery):
    await query.message.edit_text(" Entering fucking mode...")
    import asyncio
    while True:
...
async def sudoers_list(client, message: Message):
    ...
    await message.reply_video(
        video="https://files.catbox.moe/5mucn9.mp4",
...
async def check_sudo_list(client, callback_query: CallbackQuery):
    ...
    if callback_query.from_user.id not in SUDOERS:
        return await callback_query.answer("ᴛᴇʀɪ ᴍᴀᴀ ᴋɪ ᴄʜᴜᴛ 🥲 ʀɪᴏ ᴘᴀᴘᴀ sᴇ ᴘᴜᴄʜᴏ ʏʀ ʙʜᴀɪ ᴛᴜᴍʜ 😭", show_alert=True)
    else:
